...
export default function MyGear() {
  ...
  const [confirmDeleteOpened, { open: openConfirmDelete, close: closeConfirmDelete }] =
    useDisclosure(false)
  const [setupDrawerOpened, { open: openSetupDrawer, close: closeSetupDrawer }] =
    useDisclosure(false)
  const [newSetupOpened, { open: openNewSetup, close: closeNewSetup }] =
    useDisclosure(false)
  const [
    confirmDeleteSetupOpened,
    { open: openConfirmDeleteSetup, close: closeConfirmDeleteSetup },
  ] = useDisclosure(false)
  ...
  async function handleAddToSetup(productId) {
    ...
    const nextOrder =
      setupItems.length > 0 ? Math.max(...setupItems.map((i) => i.order_show)) + 1 : 1
    ...
    if (error) {
      notifications.show({
        color: 'red',
        position: 'top-center',
        message: 'Erro ao adicionar item ao setup.',
      })
    } else {
  ...
  async function handleCreateSetup() {
    ...
    if (error) {
      notifications.show({
        color: 'red',
        position: 'top-center',
        message: 'Erro ao criar setup. Tente novamente.',
      })
    } else {
      await queryClient.refetchQueries({ queryKey: ['user-setups', user.id] })
      notifications.show({
    ...
    <>
                  ...
                  >
                    ...
                    >
                      <Stack gap={2} style={{ flex: 1 }}>
                        ...
                        {subGear.filter((s) => s.parent_gear_id === item.id).length >
                          0 && (
                          <Flex gap={4} justify="center" wrap="wrap">
                            {subGear
                              .filter((s) => s.parent_gear_id === item.id)
                              .map((s) => (
                                <Avatar
                                  key={s.id}
                                  size={24}
                                  ...
                                  src={
                                    s.products?.picture
                                      ? PRODUCT_IMG + s.products.picture
                                      : undefined
                                  }
                      ...
                      <ActionIcon.Group mt="auto" pt={12} w="100%">
                        <ActionIcon
                          w="50%"
                          variant="subtle"
                          color="gray"
        ...
        {editingItem && (
          <Stack gap="md">
            ...
            <Group justify="flex-end" gap={8}>
              <Button variant="default" radius="xl" onClick={closeEditItem}>
                Cancelar
              </Button>
              <Button
      ...
      >
        ...
        <Group justify="flex-end" gap={8} mt="md">
          <Button variant="default" size="sm" onClick={closeConfirmDelete}>
            Cancelar
          </Button>
          <Button
            color="red"
          ...
          <Box>
            ...
            {newSetup.image ? (
              ...
              <Button
                ...
                variant="default"
                leftSection={
                  isUploadingSetupImg ? <Loader size={13} /> : <IconPhoto size={13} />
                }
                component="label"
      ...
      >
        <Text size="sm">
          Remover o setup <strong>{setupToDelete?.name}</strong>? Os itens do setup serão
          desvinculados, mas não removidos do seu equipamento.
        </Text>
        <Group justify="flex-end" gap={8} mt="md">
          <Button variant="default" size="sm" onClick={closeConfirmDeleteSetup}>
            Cancelar
          </Button>
          <Button
            color="red"
            size="sm"
            loading={isDeletingSetup}
            ...
            <NativeSelect
              ...
              onChange={(e) => {
                if (e.currentTarget.value) {
                  handleAddToSetup(e.currentTarget.value)
                }
              }}
            >
              <option value="">
                  ...
                  {editingSetupItem?.id === item.id && (
                    <Flex gap="sm" mt="sm">
                      <NumberInput
                        ...
                        onChange={(v) =>
                          setEditingSetupItem((prev) => ({ ...prev, order_show: v }))
                        }
                      />
                      <TextInput
                        size="xs"

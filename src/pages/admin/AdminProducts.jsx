    ...
    <Input.Wrapper label="Categoria" required error={error}>
      ...
      >
        <Combobox.Target>
          ...
          >
            {selectedLabel ?? (
              <Input.Placeholder>Selecione uma categoria</Input.Placeholder>
            )}
          </InputBase>
        </Combobox.Target>
        <Combobox.Dropdown>
...
function ProductForm({ product, brands, categories, onSave, onClose }) {
  ...
  useEffect(() => {
    ...
    supabase
      .from('product_series')
      .select('id, name')
      .eq('id_brand', Number(form.id_brand))
      .order('name')
  ...
  function validate() {
    ...
    if (
      form.description_source_url &&
      !/^https?:\/\/.+/.test(form.description_source_url)
    )
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (isEditing) {
        ...
        notifications.show({
          color: 'teal',
          message: `Produto "${form.name}" atualizado.`,
        })
    ...
    <Stack gap="md">
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        ...
        <CategoryCombobox
          ...
          onChange={(v) => setField('id_category', v)}
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        ...
        <TextInput
          label="Subtítulo"
          placeholder="Ex: Boutique Overdrive Pedal"
          value={form.subtitle}
      ...
      <TextInput
        label="Keywords"
        placeholder="Ex: guitarra, strat, single coil (separe por vírgula)"
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        ...
        <TextInput
          label="URL da fonte"
          placeholder="https://..."
          value={form.description_source_url}
          error={errors.description_source_url}
      ...
      <Alert variant="light" color="gray" icon={<IconAlertCircle size={14} />}>
        Foto genérica · de frente · sem inclinação · fundo branco ou transparente · ~22px
        de margem em cada extremidade · máx. 2mb
      </Alert>
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="xs">
        ...
        />
        <Switch
          label="Pode ser subproduto"
          checked={form.can_be_subproduct}
          size="sm"
    ...
    <Table.Tr>
      <Table.Td>
        <Group gap="sm" wrap="nowrap">
          <Avatar
            src={productImageUrl(product.picture)}
            size={36}
            radius="sm"
            color="blue"
          >
...
export default function AdminProducts() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    if (debouncedSearch)
      query = query.or(
        `name.ilike.%${debouncedSearch}%,subtitle.ilike.%${debouncedSearch}%,keywords.ilike.%${debouncedSearch}%`,
      )
  ...
  useEffect(() => {
    setPage(1)
  }, [debouncedSearch, brandFilter, categoryFilter, featuredFilter])
  ...
  function handleSaved(saved) {
    if (editingProduct)
      ...
      setProducts((prev) => [saved, ...prev].slice(0, PAGE_SIZE))
      setTotal((t) => t + 1)
    }
    closeModal()
  }
      ...
      <Stack gap="lg">
        <Group justify="space-between" align="flex-end">
          <Box>
            ...
            <Text size="sm" c="dimmed">
              {loading
                ? '...'
                : `${total.toLocaleString('pt-BR')} produto${total !== 1 ? 's' : ''} cadastrados`}
            </Text>
          </Box>
          <Group gap="sm">
            <ActionIcon
        ...
        <Group gap="sm" wrap="wrap">
          ...
          <Select
            ...
            data={[
              { value: '', label: 'Todas' },
              ...categories.map((c) => ({ value: String(c.id), label: c.name_ptbr })),
            ]}

...
import {
  ...
  Image, Divider, Progress, NumberInput, Anchor, ScrollArea,
  ThemeIcon, Input, Loader,
} from '@mantine/core'
...
import {
  ...
  IconMapPin, IconWorld, IconBrandInstagram, IconPhone,
  IconTrash, IconUsers, IconSpeakerphone, IconMusic,
  IconExternalLink,
...
const EMPTY_FORM = {
  name: '', slug: '', description: '', venue_type_id: '',
  address: '', address_number: '', neighborhood: '', zip_code: '',
  city_id: '', region_id: '',
  latitude: '', longitude: '',
  instagram_handle: '', website_url: '', phone: '',
  capacity: '', has_sound_system: false, has_backline: false,
}
...
function venueImageUrl(picture_url) {
  ...
  if (picture_url.startsWith('http')) return picture_url
...
function VenueForm({ venue, venueTypes, onSave, onClose }) {
  ...
  const [selectedCity,      setSelectedCity]      = useState(
    isEditing && venue.cities?.id ? { id: venue.cities.id, name: venue.cities.name } : null
  )
  const [cityModalOpened, { open: openCityModal, close: closeCityModal }] = useDisclosure(false)
  ...
  useEffect(() => {
    supabase.from('regions').select('id, name, uf').eq('country_id', 27).order('name')
  ...
  const handleCitySearch = useDebouncedCallback(async (query) => {
    if (!query || query.length < 2 || !form.region_id) return
  ...
  function validate() {
    ...
    if (form.latitude  && isNaN(Number(form.latitude)))  errs.latitude  = 'Latitude inválida'
    if (form.longitude && isNaN(Number(form.longitude))) errs.longitude = 'Longitude inválida'
    return errs
  }
  ...
  const previewUrl = picFileName
    ...
    : (isEditing && venue.picture_url ? venueImageUrl(venue.picture_url) : null)
  ...
  return (
    <Stack gap="md">
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        <TextInput
          label="Nome" placeholder="Ex: Blue Note São Paulo" required
          value={form.name} error={errors.name}
          onChange={(e) => setField('name', e.currentTarget.value)}
        />
        <TextInput
          label="Slug" placeholder="Ex: blue-note-sao-paulo" required
      ...
      <Select
        ...
        value={form.venue_type_id} clearable
        onChange={(v) => setField('venue_type_id', v ?? '')}
      />

      <Textarea
        label="Descrição" placeholder="Descrição do local..."
        value={form.description} rows={3} maxLength={3000}
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        <Select
          ...
          data={regions.map(r => ({ value: String(r.id), label: `${r.name} (${r.uf})` }))}
          value={form.region_id} clearable searchable
        ...
        <Input.Wrapper label="Cidade">
          <Input
            pointer
            readOnly
            placeholder={form.region_id ? 'Buscar cidade...' : 'Selecione o estado primeiro'}
            ...
            rightSection={
              selectedCity
                ...
                : form.region_id ? <IconSearch size={15} /> : undefined
            }
            onClick={() => { if (form.region_id) openCityModal() }}
          />
        </Input.Wrapper>
      </SimpleGrid>
      ...
      <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
        <TextInput
          label="Endereço" placeholder="Nome da rua / avenida"
          value={form.address}
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        <TextInput
          label="Latitude" placeholder="Ex: -23.5505"
        ...
        />
        <TextInput
          label="Longitude" placeholder="Ex: -46.6333"
      ...
      <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
        <TextInput
          ...
          value={form.website_url} error={errors.website_url}
          onChange={(e) => setField('website_url', e.currentTarget.value)}
        />
        <TextInput
          label="Instagram" placeholder="@bluenotesp"
      ...
      <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
        <NumberInput
          label="Capacidade" placeholder="Ex: 500"
      ...
      {previewUrl ? (
        <Group gap="md" align="center">
          <Image
            src={previewUrl} w={120} h={80} fit="cover" radius="md"
            ...
            <ScrollArea h={220} type="auto">
              <Stack gap={0}>
                {cityResults.map(city => (
                  <Box key={city.id}>
                    <Anchor
                      size="sm" py={8} display="block"
                      underline="never" c="inherit"
                      onClick={() => {
    ...
    <Table.Tr>
      <Table.Td>
        <Group gap="sm" wrap="nowrap">
          <Avatar
            ...
            size={36} radius="sm" color="orange"
          >
            <IconMapPin size={18} />
          </Avatar>
          <Box>
            <Text size="sm" fw={500} lineClamp={1}>{venue.name}</Text>
            <Text size="xs" c="dimmed" ff="monospace" lineClamp={1}>{venue.slug}</Text>
          </Box>
        </Group>
      </Table.Td>
      <Table.Td visibleFrom="sm">
        <Text size="xs" c="dimmed">{typeName ?? '—'}</Text>
      </Table.Td>
      ...
      <Table.Td visibleFrom="lg">
        <Group gap={4} wrap="nowrap">
          ...
          {venue.capacity && (
            <Text size="xs" c="dimmed">{venue.capacity.toLocaleString('pt-BR')} pax</Text>
          )}
        </Group>
      </Table.Td>
      <Table.Td visibleFrom="lg">
      ...
      <Table.Td>
        <Tooltip label="Editar">
          <ActionIcon variant="subtle" color="gray" size="sm" onClick={() => onEdit(venue)}>
            <IconPencil size={14} />
          </ActionIcon>
        </Tooltip>
      </Table.Td>
...
export default function AdminVenues() {
  ...
  const fetchVenues = useCallback(async (currentPage) => {
    setLoading(true)
    ...
    if (typeFilter)         query = query.eq('venue_type_id', Number(typeFilter))
    if (soundFilter !== '')    query = query.eq('has_sound_system', soundFilter === 'true')
    if (backlineFilter !== '') query = query.eq('has_backline', backlineFilter === 'true')

    const { data, count, error } = await query

    if (error) {
      notifications.show({ color: 'red', message: 'Erro ao carregar venues: ' + error.message })
    } else {
  ...
  function openCreate() { setEditingVenue(null); openModal() }
  function openEdit(venue) { setEditingVenue(venue); openModal() }

  function handleSaved(saved) {
    if (editingVenue) setVenues(prev => prev.map(v => v.id === saved.id ? saved : v))
    else { setVenues(prev => [saved, ...prev].slice(0, PAGE_SIZE)); setTotal(t => t + 1) }
    closeModal()
  }
    ...
    <>
      <Stack gap="lg">
        ...
        <Group justify="space-between" align="flex-end">
          <Box>
            <Title order={3} fw={500}>Venues</Title>
            <Text size="sm" c="dimmed">
              {loading ? '...' : `${total.toLocaleString('pt-BR')} venue${total !== 1 ? 's' : ''} cadastrado${total !== 1 ? 's' : ''}`}
            </Text>
          </Box>
          <Group gap="sm">
            <ActionIcon variant="subtle" color="gray" onClick={() => fetchVenues(page)} loading={loading}>
              <IconRefresh size={16} />
            </ActionIcon>
        ...
        <Group gap="sm" wrap="wrap">
          ...
          <Select
            placeholder="Tipo"
            data={[{ value: '', label: 'Todos os tipos' }, ...venueTypes.map(t => ({ value: String(t.id), label: t.name }))]}
            value={typeFilter} size="sm" w={180} clearable
            onChange={(v) => setTypeFilter(v ?? '')}
          />
          <Select
            placeholder="Som"
            data={[{ value: '', label: 'Som: todos' }, { value: 'true', label: 'Com sistema de som' }, { value: 'false', label: 'Sem sistema de som' }]}
            value={soundFilter} size="sm" w={180} clearable
            onChange={(v) => setSoundFilter(v ?? '')}
          />
          <Select
            placeholder="Backline"
            data={[{ value: '', label: 'Backline: todos' }, { value: 'true', label: 'Com backline' }, { value: 'false', label: 'Sem backline' }]}
            value={backlineFilter} size="sm" w={165} clearable
            onChange={(v) => setBacklineFilter(v ?? '')}
          />
          {hasActiveFilters && (
            <Tooltip label="Limpar filtros">
              <ActionIcon variant="light" color="gray" size="sm"
                onClick={() => { setTypeFilter(''); setSoundFilter(''); setBacklineFilter('') }}>
                <IconFilter size={14} />
              </ActionIcon>
            </Tooltip>
          )}
              ...
              {loading
                ? Array.from({ length: 8 }).map((_, i) => (
                    <Table.Tr key={i}>
                      ...
                      <Table.Td visibleFrom="lg"><Skeleton height={10} width={80} /></Table.Td>
                      <Table.Td visibleFrom="lg"><Skeleton height={10} width={120} /></Table.Td>
                      <Table.Td><Skeleton height={24} width={28} /></Table.Td>
                    </Table.Tr>
                  ))
                : venues.length === 0
                ? <Table.Tr><Table.Td colSpan={6}><Text c="dimmed" size="sm" ta="center" py="xl">Nenhum venue encontrado.</Text></Table.Td></Table.Tr>
                : venues.map(venue => (
                    <VenueRow
                      key={venue.id} venue={venue}
                      venueTypes={venueTypes} onEdit={openEdit}
                    />
                  ))
              }
            </Table.Tbody>

...
import {
  ...
  Avatar, ActionIcon, Drawer, Divider, Skeleton, Table,
  Pagination, Tooltip, CopyButton, ThemeIcon, SimpleGrid,
  Card, Anchor, Button, Switch, Modal, Textarea, ScrollArea,
  Box, Indicator, SegmentedControl, Alert,
} from '@mantine/core'
import { useDebouncedValue, useDisclosure } from '@mantine/hooks'
import { notifications } from '@mantine/notifications'
import {
  IconSearch, IconCrown, IconCopy, IconCheck, IconExternalLink,
  IconShieldCheck, IconMapPin, IconCalendar, IconPhone, IconWorld,
  IconRefresh, IconFilter, IconUserShield, IconClock, IconAlertTriangle,
...
function planBadge(plan, origin, expiresAt) {
  if (plan === 'Pro') {
    ...
    if (expired) return <Badge size="xs" color="red" variant="light">Pro · Expirado</Badge>
    const color = origin === 'courtesy' ? 'orange' : origin === 'partner' ? 'teal' : 'yellow'
    const label = origin === 'courtesy' ? 'Pro · Cortesia' : origin === 'partner' ? 'Pro · Parceiro' : 'Pro'
    return <Badge size="xs" color={color} variant="light">{label}</Badge>
  }
  return <Badge size="xs" color="gray" variant="light">Free</Badge>
}
...
function expiryInfo(expiresAt) {
  if (!expiresAt) return { label: 'Sem prazo (permanente)', color: 'teal', expired: false }
  const now  = new Date()
  const exp  = new Date(expiresAt)
  const diff = Math.ceil((exp - now) / (1000 * 60 * 60 * 24)) // dias restantes

  if (diff < 0)  return { label: `Expirou em ${formatDate(expiresAt)}`, color: 'red',    expired: true  }
  if (diff === 0) return { label: 'Expira hoje',                         color: 'red',    expired: false }
  if (diff <= 7)  return { label: `Expira em ${diff} dia${diff > 1 ? 's' : ''}`, color: 'orange', expired: false }
  if (diff <= 30) return { label: `Expira em ${diff} dias`,              color: 'yellow', expired: false }
    ...
    <Table.Tr style={{ cursor: 'pointer' }} onClick={() => onSelect(user)}>
      <Table.Td>
        <Group gap="sm" wrap="nowrap">
          <Indicator disabled={!user.is_live} color="green" size={8} offset={3} withBorder>
            ...
            <Group gap={4} wrap="nowrap">
              ...
              {user.is_verified && (
                <Tooltip label="Verificado">
                  <ThemeIcon size={14} color="blue" variant="transparent" style={{ flexShrink: 0 }}>
              ...
              {user.is_admin && (
                <Tooltip label="Admin">
                  <ThemeIcon size={14} color="violet" variant="transparent" style={{ flexShrink: 0 }}>
      ...
      <Table.Td visibleFrom="lg">
        <Text size="xs" c="dimmed" lineClamp={1}>{user.title ?? '—'}</Text>
      </Table.Td>
    </Table.Tr>
  )
}
...
function UserDrawer({ user, opened, onClose, onUpdate }) {
  ...
  async function grantPro() {
    ...
    if (days === 0) {
      ...
      ;({ error } = await supabase
        .from('profiles')
        .update({ plan: 'Pro', plan_origin: 'courtesy', plan_expires_at: null, plan_note: note })
        .eq('id', user.id))
    } else {
    ...
    setSaving(false)
    if (error) {
      notifications.show({ color: 'red', message: 'Erro ao conceder Pro: ' + error.message })
    } else {
      ...
      onUpdate({ ...user, plan: 'Pro', plan_origin: 'courtesy', plan_expires_at: expiresAt, plan_note: note })
  ...
  async function revokePro() {
    ...
    setSaving(false)
    if (error) {
      notifications.show({ color: 'red', message: 'Erro ao revogar Pro: ' + error.message })
    } else {
      notifications.show({ color: 'orange', message: `Plano Pro revogado de @${user.username}.` })
      onUpdate({ ...user, plan: 'Free', plan_origin: 'none', plan_expires_at: null, plan_note: note })
      setConfirmModal(null)
    }
  }
  ...
  async function toggleAdmin() {
    ...
    setSaving(false)
    if (error) {
      notifications.show({ color: 'red', message: 'Erro ao alterar admin: ' + error.message })
    } else {
      notifications.show({ color: 'violet', message: `Status admin atualizado para @${user.username}.` })
  ...
  return (
    <>
      ...
      >
        <Stack gap="md">
          ...
          <Group gap="xs">
            {planBadge(user.plan, user.plan_origin, user.plan_expires_at)}
            {user.is_verified && (
              <Badge size="xs" color="blue" variant="light" leftSection={<IconShieldCheck size={10} />}>
                Verificado
              </Badge>
            )}
            {user.is_admin && (
              <Badge size="xs" color="violet" variant="light" leftSection={<IconUserShield size={10} />}>
                Admin
              </Badge>
            )}
            {user.is_legend && <Badge size="xs" color="yellow" variant="light">Lenda</Badge>}
            {user.is_live   && <Badge size="xs" color="green"  variant="dot">Ao vivo</Badge>}
          </Group>
          ...
          <SimpleGrid cols={2} spacing="xs">
            <Box>
              <Text size="xs" c="dimmed" mb={2}>UUID</Text>
              <Group gap={4} wrap="nowrap">
                <Text size="xs" ff="monospace" lineClamp={1} style={{ maxWidth: 140 }}>{user.id}</Text>
            ...
            <Box>
              <Text size="xs" c="dimmed" mb={2}>Cadastro</Text>
              <Group gap={4}>
            ...
            {user.title && (
              <Box style={{ gridColumn: 'span 2' }}>
                <Text size="xs" c="dimmed" mb={2}>Título</Text>
                <Text size="sm">{user.title}</Text>
              </Box>
            )}
            {user.bio && (
              <Box style={{ gridColumn: 'span 2' }}>
                <Text size="xs" c="dimmed" mb={2}>Bio</Text>
                <Text size="xs" c="dimmed" lineClamp={4}>{user.bio}</Text>
              </Box>
            )}
          </SimpleGrid>
          ...
          <Card withBorder radius="md" padding="sm">
            <Group justify="space-between" mb="xs">
              <Text size="sm" fw={500}>Plano atual</Text>
              {planBadge(user.plan, user.plan_origin, user.plan_expires_at)}
            </Group>
            ...
            {isPro && isCourtesy && expiry && (
              <Group gap={6} mb="sm">
                <IconClock size={13} style={{ color: `var(--mantine-color-${expiry.color}-6)` }} />
            ...
            {isPro && isCourtesy && expiry?.expired && (
              <Alert
                variant="light" color="red" mb="sm"
                icon={<IconAlertTriangle size={14} />}
              >
                <Text size="xs">
                  Esta cortesia expirou. O usuário ainda aparece como Pro no banco,
                  mas <code>effective_plan()</code> já retorna Free para o app.
                  Reconcedam ou revoguem abaixo.
                </Text>
              </Alert>
            )}
            ...
            {!isPro ? (
              ...
              <Group gap="xs">
                ...
                <Button size="xs" variant="light" color="red" onClick={() => setConfirmModal('revoke')}>
                  Revogar
                </Button>
              </Group>
            ) : (
              <Text size="xs" c="dimmed">Plano pago — não gerenciável aqui.</Text>
            )}
          </Card>
          ...
          <Card withBorder radius="md" padding="sm">
            <Group justify="space-between">
              <Box>
                <Text size="sm" fw={500}>Acesso admin</Text>
                <Text size="xs" c="dimmed">Permite acessar o backoffice</Text>
              </Box>
              <Switch
      ...
      <Modal
        ...
        title={isCourtesy ? `Renovar cortesia — @${user.username}` : `Conceder Pro (cortesia) — @${user.username}`}
        size="sm" centered
      >
        <Stack gap="md">
          <Box>
            <Text size="xs" c="dimmed" mb={8}>Duração da cortesia</Text>
            <SegmentedControl
              fullWidth
              size="xs"
              value={durationDays}
          ...
          {durationDays === '0' && (
            <Text size="xs" c="dimmed">
              Sem data de expiração. A cortesia será permanente até ser revogada manualmente.
            </Text>
          )}
          ...
          <Group justify="flex-end">
            <Button variant="default" size="xs" onClick={() => setConfirmModal(null)}>Cancelar</Button>
      ...
      >
        <Stack gap="md">
          <Text size="sm">
            Isso vai rebaixar <strong>@{user.username}</strong> para o plano <strong>Free</strong>{' '}
            e limpar o prazo de expiração. Tem certeza?
          </Text>
          <Textarea
          ...
          <Group justify="flex-end">
            <Button variant="default" size="xs" onClick={() => setConfirmModal(null)}>Cancelar</Button>
            <Button size="xs" color="red" loading={saving} onClick={revokePro}>Revogar</Button>
          </Group>
        </Stack>
      </Modal>
      ...
      >
        <Stack gap="md">
          ...
          <Group justify="flex-end">
            <Button variant="default" size="xs" onClick={() => setConfirmModal(null)}>Cancelar</Button>
            <Button size="xs" color="violet" loading={saving} onClick={toggleAdmin}>Confirmar</Button>
          </Group>
        </Stack>
      </Modal>
    </>
...
export default function AdminUsers() {
  ...
  const fetchUsers = useCallback(async (currentPage) => {
    ...
    let query = supabase
      ...
      .select(
        `id, full_name, username, avatar, plan, plan_origin, plan_expires_at, plan_note,
         is_verified, is_admin, is_legend, is_live, title, bio,
    ...
    if (debouncedSearch)       query = query.or(`full_name.ilike.%${debouncedSearch}%,username.ilike.%${debouncedSearch}%`)
    if (planFilter)            query = query.eq('plan', planFilter)
    if (originFilter)          query = query.eq('plan_origin', originFilter)
    if (verifiedFilter !== '') query = query.eq('is_verified', verifiedFilter === 'true')

    const { data, count, error } = await query
    if (error) notifications.show({ color: 'red', message: 'Erro ao carregar usuários: ' + error.message })
    ...
    <>
      <Stack gap="lg">
        ...
        <Group justify="space-between" align="flex-end">
          <Box>
            <Title order={3} fw={500}>Usuários</Title>
            <Text size="sm" c="dimmed">
              {loading ? '...' : `${total.toLocaleString('pt-BR')} usuários cadastrados`}
            </Text>
          </Box>
          <ActionIcon variant="subtle" color="gray" onClick={() => fetchUsers(page)} loading={loading}>
            <IconRefresh size={16} />
          </ActionIcon>
        </Group>
        ...
        <Group gap="sm" wrap="wrap">
          ...
          />
          <Select
            placeholder="Plano" data={PLAN_OPTIONS}
            value={planFilter} size="sm" w={150} clearable
            onChange={(v) => setPlanFilter(v ?? '')}
          />
          <Select
            placeholder="Origem" data={ORIGIN_OPTIONS}
            value={originFilter} size="sm" w={160} clearable
            onChange={(v) => setOriginFilter(v ?? '')}
          />
          <Select
            placeholder="Verificação" data={VERIFIED_OPTIONS}
            value={verifiedFilter} size="sm" w={150} clearable
          ...
          {hasActiveFilters && (
            <Tooltip label="Limpar filtros">
              <ActionIcon
                ...
                onClick={() => { setPlanFilter(''); setOriginFilter(''); setVerifiedFilter('') }}
              >
                <IconFilter size={14} />
              </ActionIcon>
            </Tooltip>
              ...
              {loading
                ? Array.from({ length: 8 }).map((_, i) => (
                    <Table.Tr key={i}>
                      ...
                      <Table.Td visibleFrom="sm"><Skeleton height={10} width={180} /></Table.Td>
                      <Table.Td><Skeleton height={18} width={80} radius="xl" /></Table.Td>
                      <Table.Td visibleFrom="md"><Skeleton height={10} width={90} /></Table.Td>
                      <Table.Td visibleFrom="lg"><Skeleton height={10} width={120} /></Table.Td>
                    </Table.Tr>
                  ))
                : users.length === 0
                ? (
                    <Table.Tr>
                      <Table.Td colSpan={5}>
                        <Text c="dimmed" size="sm" ta="center" py="xl">Nenhum usuário encontrado.</Text>
                      </Table.Td>
                    </Table.Tr>
                  )
                : users.map(user => (

...
export default function GigsDashboard() {
  ...
  return (
    <>
      ...
      <Box mt="lg" mb="xs" id="gigs-list-view">
        <Group gap={6} mb="sm">
          <IconCalendarEvent size={18} stroke={1.8} style={{ opacity: 0.6 }} />
          <Title order={3} fw={600} fz="lg" lh={1}>
            {isSelectedToday
              ? 'Hoje'
              : dayjs(miniCalendarCurrentDate).format('DD [de] MMMM')}
          </Title>
        </Group>
          ...
          <Stack gap="xs" mt="sm">
            {gigsForSelectedDay.map((item) => (
              <Paper key={item.id} withBorder p="xs" radius="md">
                ...
                <Text size="xs" c="dimmed">
                  {item.gig.date ? dayjs(item.gig.date).format('HH:mm') : ''}
                </Text>
              </Paper>
            ))}
          </Stack>
